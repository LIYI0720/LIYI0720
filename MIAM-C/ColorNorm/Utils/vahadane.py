...
class vahadane(object):
    ...
    def __init__(self, STAIN_NUM=2, THRESH=0.9, LAMBDA1=0.01, LAMBDA2=0.01, ITER=100, fast_mode=0, getH_mode=0, verbose = False):
    ...
    def SPCN_2(self, img_s, img_t, Ws, Wt):
        V0s, Vs = self.getV(img_s)
        Hs = self.getH(V0s, Ws)
        V0t, Vt = self.getV(img_t)
        Ht = self.getH(V0t, Wt)
        Hs_RM = np.percentile(Hs, 99)
        Ht_RM = np.percentile(Ht, 99)
        Hs_norm = Hs * Ht_RM / Hs_RM
        Vs_norm = np.dot(Wt, Hs_norm)
        Is_norm = 255 * np.exp(-1 * Vs_norm)
        I = Is_norm.T.reshape(img_s.shape).astype(np.uint8)
        return I
